...
def scatterplot(great_df:pd.DataFrame,colname_x,colname_y,title:str="",minus_log10=True):
    ...
    if minus_log10 :
        ...
        sp(data=great_df,x=f"-log({colname_x})",y=f"-log({colname_y})").set_title(title)
    else : 
        sp(data=great_df,x=colname_x,y=colname_y).set_title(title)
    plt.show()
